...
public class ProgramToData implements ProgramToDataConstants {
  ...
  public static Program run(String input) throws ParseException {
    Program program;
    varnum = 0;
    program = new ProgramToData(new java.io.StringReader(input)).start();
    return program;
  }

  public String toString(Program p) {
        String programString = p.print;
        String indented = "";
        int indent = 0;
        String[] lines;

        lines = programString.split(System.getProperty("line.separator"));
  ...
  final public Program start() throws ParseException {varList = new ArrayList<Var>();
  Program p;
    p = program();
    jj_consume_token(0);
{if ("" != null) return p;}
...
class Macro extends Cmd {
  public Macro(Var target, String path, Expr input) {
    ...
    try {
      ...
      String text = scanner.useDelimiter("\u005c\u005cA").next();
      scanner.close();
      String macroProgram = ProgramToData.parser.run(text).toString();
